...
metadata  <- read_csv(paste(raw.dir, 
  ...
  # mutate(period = as.character(period)) %>%                                   # Turn this line on if you have numerical sample names 
...
points <- read.delim(paste(raw.dir, paste0(study, "_Dot Point Measurements.txt"), sep = "/"),
  ...
  ga.clean.names() %>%                                                          # Tidy the column names using GlobalArchive function
  # mutate(period = as.character(period)) %>%                                   # Turn on if you have numerical sample names
  select(period, image.row, image.col, broad, morphology, type) %>%             # Select only these columns to keep
  glimpse()                                                                     # Preview the data
...
relief <- read.delim(paste(raw.dir, 
                           ...
                           sep = "/"), header = T, skip = 4, 
                     stringsAsFactors = F) %>%                                  # Read in the file
  ga.clean.names() %>%                                                          # Tidy the column names using GlobalArchive function
  # mutate(period = as.character(period)) %>%                                   # Turn on if you have numerical sample names
  select(period, image.row, image.col, broad, morphology, type, relief) %>%     # Select only these columns to keep
  glimpse() # Preview the data
...
image.list <- dir(image.dir) %>%                                                # Selects the directory where your habitat images are stored
  ...
  separate(image.name, sep = "_", into = c(NA, "period"),                       # Parse period (sample) from image name
           extra = "drop", remove = F)

# Create a data frame to check habitat annotations against using original metadata
qaqc.habitat <- metadata %>%
  dplyr::select(period, date) %>%                                               # Select only these columns to keep
...
qaqc.all <- metadata %>%
  select(period, date) %>%                                                      # Select only these columns
...
write.csv(habitat.missing.image.not.annotated,
          paste(error.dir, paste(study,"habitat.missing.image.not.annotated.csv", 
                                 sep = "."), sep = "/"),row.names = F) 
...
write.csv(relief.missing.image.not.annotated,
          paste(error.dir, paste(study,"relief.missing.image.not.annotated.csv", 
                                 sep = "."), sep = "/"), row.names = F)
...
broad.points <- habitat %>%
  ...
  mutate(total.points.annotated = rowSums(.[,2:(ncol(.))],na.rm = T )) %>%      # Take row sums of all data columns
...
habitat.broad.points <- metadata %>%
  left_join(broad.points, by = "period") %>%                                    # Join metadata with habitat data
  left_join(relief.grid)                                                        # And relief

habitat.detailed.points <- metadata %>%
  left_join(detailed.points, by = "period") %>%                                 # Join metadata with habitat data
  left_join(relief.grid)                                                        # And relief

habitat.broad.percent <- metadata %>%
  left_join(broad.percent.cover, by = "period") %>%                             # Join metadata with habitat data
  left_join(relief.grid)                                                        # And relief

habitat.detailed.percent <- metadata %>%
  left_join(detailed.percent.cover, by = "period") %>%                          # Join metadata with habitat data
  left_join(relief.grid)                                                        # And relief
...
broad.rel.plot <- relief %>%
  ...
  dplyr::group_by(period, relief.rank) %>%
...
pie.chart <- leaflet() %>%                                                      # Create a leaflet plot
  ...
  addLayersControl(baseGroups = c("Open Street Map", "World Imagery"), 
                   options = layersControlOptions(collapsed = F)) %>%           # Add controls to switch between layers
...
if (nrow(overzero)) {                                                           # Add spatial bubble plots if the data is greater than zero
  bubble.plot <- bubble.plot %>%
    addCircleMarkers(data = overzero, lat = ~ latitude, lng = ~ longitude,      # Add the bubble plots
                     ...
                     label = ~ as.character(period))                            # Format the points and add labels for sample code
}
if (nrow(equalzero)) {                                                          # Add spatial bubble plots if the data is equal to zero
  bubble.plot <- bubble.plot %>%
    addCircleMarkers(data = equalzero, lat = ~ latitude, lng = ~ longitude,     # Add the bubble plots
                     ...
                     fillOpacity = 0.5, color = "white", stroke = F, 
                     label = ~as.character(period))                             # Format the points and add labels for sample code
}
